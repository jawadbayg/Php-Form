...
<div class="container mt-5">
    ...
    $name = $email = $age = $country = $password = $gender = "";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($nameErr) && empty($emailErr) && empty($ageErr) && empty($countryErr) && empty($passwordErr) && empty($skillsErr) && empty($genderErr)) {
            $_SESSION["name"] = $name;
            $_SESSION["email"] = $email;
            $_SESSION["age"] = $age;
            $_SESSION["country"] = $country;
